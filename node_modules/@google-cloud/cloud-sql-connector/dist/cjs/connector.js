...
exports.cooldownBackoff = cooldownBackoff;
...
const sql_data_client_1 = require("./sql-data-client");
function cooldownBackoff(base, attempt) {
    const multi = 1.618;
    ...
    getInstance(opts) {
        ...
        if (!connectionInstance || !connectionInstance.instance) {
            throw new errors_1.CloudSQLConnectorError({
                message: `Cannot find info for instance: ${opts.instanceConnectionName || opts.domainName}`,
...
class Connector {
    constructor(opts = {}) {
        var _a;
        this.sqlDataTunnels = new Map();
        this.sqlDataStates = new Map();
        this.sqlDataFallbackIpTypes = new Map();
        ...
        this.resourceExhaustedCooldownPeriod =
            (_a = opts.resourceExhaustedCooldownPeriod) !== null && _a !== void 0 ? _a : 5000;
    }
    getSqlDataState(connectionName) {
    ...
    async getOptions(opts) {
        ...
        const instanceInfo = await (0, parse_instance_connection_name_1.resolveInstanceName)(opts.instanceConnectionName, opts.domainName, this.sqlAdminFetcher);
        const connectionName = `${instanceInfo.projectId}:${instanceInfo.regionId}:${instanceInfo.instanceId}`;
        let ipType = opts.ipType || ip_addresses_1.IpAddressTypes.PUBLIC;
        const state = this.getSqlDataState(connectionName);
        if (ipType === ip_addresses_1.IpAddressTypes.SQL_DATA) {
            if (!state.allowed) {
                ipType =
        ...
        if (ipType === ip_addresses_1.IpAddressTypes.SQL_DATA) {
            return await this.developerEditionOptions(connectionName, instances, resolvedOpts, instanceInfo);
        }
        return {
            stream: () => this.createDirectSocket(instances, resolvedOpts),
        };
    }
    async getFallbackIpType(instanceInfo) {
        const metadata = await this.sqlAdminFetcher.getInstanceMetadata(instanceInfo);
        if (metadata.ipAddresses.private) {
            return ip_addresses_1.IpAddressTypes.PRIVATE;
        }
        if (metadata.ipAddresses.psc) {
            return ip_addresses_1.IpAddressTypes.PSC;
        }
        if (metadata.ipAddresses.public) {
            return ip_addresses_1.IpAddressTypes.PUBLIC;
        }
        return ip_addresses_1.IpAddressTypes.PUBLIC;
    }
    createDirectSocket(instances, opts) {
    ...
    async developerEditionOptions(connectionName, instances, opts, instanceInfo) {
        var _a;
        const state = this.getSqlDataState(connectionName);
        const cooldownPeriod = (_a = opts.resourceExhaustedCooldownPeriod) !== null && _a !== void 0 ? _a : this.resourceExhaustedCooldownPeriod;
        ...
        if (!tunnel) {
            ...
            tunnel = new sql_data_client_1.SqlDataClient({
                ...
                keepAliveTimeMs: opts.sqlDataKeepAliveTimeMs || this.sqlDataKeepAliveTimeMs,
        ...
        return {
            stream: () => {
                if (!state.allowed) {
                    const fallbackIpType = this.sqlDataFallbackIpTypes.get(connectionName) ||
                        ip_addresses_1.IpAddressTypes.PUBLIC;
                    const fallbackOpts = {
                        ...opts,
                        ipType: fallbackIpType,
                    };
