...
exports.isCodebasePartiallyFiltered = isCodebasePartiallyFiltered;
exports.parseDeleteFilters = parseDeleteFilters;
exports.detectCodebaseAndIdCollisions = detectCodebaseAndIdCollisions;
...
function endpointMatchesFilter(endpoint, filter) {
    if (filter.codebase) {
        const endpointCodebase = endpoint.codebase || projectConfig_1.DEFAULT_CODEBASE;
...
function isCodebasePartiallyFiltered(codebase, filters) {
    if (!filters) {
        return false;
    }
    return filters.some((f) => (!f.codebase || f.codebase === codebase) && !!f.idChunks && f.idChunks.length > 0);
}
function parseDeleteFilters(filters, activeCodebases) {
    ...
    return filters.flatMap((f) => {
        ...
        return parsed.map((filter) => !f.includes(":") && filter.codebase === projectConfig_1.DEFAULT_CODEBASE && filter.idChunks
            ? { idChunks: filter.idChunks }
            : filter);
    });
}
function detectCodebaseAndIdCollisions(filters, activeCodebases, allEndpoints, defaultCodebase = projectConfig_1.DEFAULT_CODEBASE) {
    const collisions = [];
