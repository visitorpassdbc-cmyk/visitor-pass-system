...
class Fabricator {
    ...
    async applyPlan(plan) {
        ...
        const secretAccessPromises = Object.values(plan).flatMap((codebasePlan) => Object.entries(codebasePlan.secretAccessPlan || {}).map(([secret, serviceAccounts]) => this.executor.run(() => ensure.grantSecretAccess({
            ...
            retryPredicates: [executor_1.isTransientError, executor_1.isServiceAccountPropagationError],
        })));
    ...
    async createV1Function(endpoint, scraper) {
        ...
        const resultFunction = await this.functionExecutor
            .run(() => scraper.withToken(async (token) => {
    ...
    async updateV2Function(endpoint, scraper) {
        ...
        const resultFunction = await this.functionExecutor
            ...
            retryPredicates: [
                executor_1.isTransientError,
                executor_1.isCloudRunResourceExhausted,
                executor_1.isServiceAccountPropagationError,
            ],
        })
            .catch((err) => {
            logger_1.logger.error(err.message);
