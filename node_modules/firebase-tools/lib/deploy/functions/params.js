...
async function ensureSecret(secretParam, projectId, nonInteractive, force) {
    ...
    if (!metadata.secret) {
        ...
        const promptMessage = `The value for this secret (${secretParam.name}) will be stored in Cloud Secret Manager (https://cloud.google.com/secret-manager/pricing) as ${resourceId}. Enter ${secretParam.format === "json" ? "a JSON value" : "a value"} for ${secretParam.label || secretParam.name}:`;
...
async function promptList(param, projectId, resolvedDefault) {
    ...
    if (isSelectInput(param.input)) {
        ...
        return promptSelectMultiple(prompt, param.input, resolvedDefault, param.input.multiSelect.nonEmpty, (res) => res);
    }
        ...
        return promptText(prompt, param.input, resolvedDefault, param.input.text.nonEmpty, (res) => {
...
async function promptBooleanParam(param, resolvedDefault) {
    ...
    if (isSelectInput(param.input)) {
        ...
        return promptText(prompt, param.input, resolvedDefault, false, isTruthyInput);
    }
    else if (isResourceInput(param.input)) {
...
async function promptStringParam(param, projectId, resolvedDefault) {
    ...
    if (isResourceInput(param.input)) {
        ...
        return promptText(prompt, param.input, resolvedDefault, param.input.text.nonEmpty, (res) => res);
    }
    else {
        (0, functional_1.assertExhaustive)(param.input);
    }
