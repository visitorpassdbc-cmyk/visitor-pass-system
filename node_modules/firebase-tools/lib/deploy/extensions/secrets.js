...
exports.secretHasExtensionsLabel = secretHasExtensionsLabel;
exports.transferSecretToKits = transferSecretToKits;
...
async function secretHasExtensionsLabel(projectId, secretName) {
    try {
        const have = await secretManager.getSecret(projectId, secretName);
        return secretUtils.SECRET_LABEL in have.labels;
    }
    catch (err) {
        throw new error_1.FirebaseError(`Error when retrieving current state of migrating secret ${projectId}/${secretName}: ${err instanceof Error ? err.message : String(err)}`, { original: err instanceof Error ? err : undefined });
    }
}
async function transferSecretToKits(projectId, secretName) {
    const newLabels = {};
    try {
        const have = await secretManager.getSecret(projectId, secretName);
    ...
    newLabels[secretManager_1.FIREBASE_MANAGED] = "functions";
