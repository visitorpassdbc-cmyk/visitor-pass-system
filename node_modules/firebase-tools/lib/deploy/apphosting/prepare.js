...
async function default_1(context, options) {
    ...
    for (const cfg of Object.values(context.backendConfigs)) {
        ...
        await injectEnvVarsFromApphostingConfig(configs.filter((c) => c.backendId === cfg.backendId), options, buildEnv, runtimeEnv, runConfigs);
...
async function injectEnvVarsFromApphostingConfig(configs, options, buildEnv, runtimeEnv, runConfigs) {
    for (const cfg of configs) {
        const rootDir = options.projectRoot || process.cwd();
        const appDir = path.join(rootDir, cfg.rootDir || "");
        let yamlConfig = yaml_1.AppHostingYamlConfig.empty();
        try {
            yamlConfig = await (0, config_1.getAppHostingConfiguration)(appDir, false);
        }
        catch (e) {
        ...
        if (runConfigs && yamlConfig.runConfig) {
            const apiRunConfig = (0, yaml_1.toApiRunConfig)(yamlConfig.runConfig);
            if (apiRunConfig) {
