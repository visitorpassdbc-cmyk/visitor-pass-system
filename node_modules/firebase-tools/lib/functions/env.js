...
exports.hasProjectEnv = hasProjectEnv;
...
exports.writeResolvedSecretRefs = writeResolvedSecretRefs;
...
function keyConflictsWithReservedPrefixes(key) {
...
function keyPermittedByKnownPrefix(key, prefix) {
    ...
    if (!prefix.endsWith("_") && key !== prefix) {
        throw new KeyValidationError(key, `Key ${key} conflicts with known key ${prefix} that does not permit a suffix`);
    }
    return true;
}
...
function hasProjectEnv(dir, projectId, projectAlias) {
    ...
    return ((!!projectId && fs.existsSync(path.join(dir, `.env.${projectId}`))) ||
...
function loadUserEnvs(opts) {
    ...
    const relativeEnvFiles = envFiles.map((f) => path.relative(opts.projectDir, path.join(configDir, f)));
    (0, utils_1.logBullet)(clc.cyan(clc.bold("functions: ")) +
        `Loaded environment variables from ${relativeEnvFiles.join(", ")}`);
    return envs;
}
function loadFirebaseEnvs(firebaseConfig, projectId, kitInstanceId) {
    const envs = {
...
function writeResolvedSecretRefs(resolvedSecretRefs, haveSecretRefs, userEnvOpt) {
    const toWrite = {};
